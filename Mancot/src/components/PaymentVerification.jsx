...
const { count, setCount } = useContext(CartContext);
  ...
  useEffect(()=>{
    const fetchCheckout=async()=>{
      const response=await axios.get(`http://localhost:5000/api/paymentDetails/${orderId}`)
  ...
  useEffect(()=>{
    const DeleteCart=async()=>{
      const response=await axios.delete(`http://localhost:5000/api/DeleteCart/${CheckOut.cart}`)
